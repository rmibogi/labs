...
try:
    ...
    n_B = ceil(n/2)         # размер подматрицы B
    ...
    else:                                                                       # иначе, меняем B и E местами несимметрично, если кол-во элементов в изначальной матрице нечетное центральный элемент также меняем
        for i in range(ceil(n/2)):
            for j in range(ceil(n/2)):
